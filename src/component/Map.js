...
const Map = (props) => {
  const [gridSize, setGridSize] = useState(40);  // 降低預設 gridSize
  const {
    constructionsData,
    mapParameters,
        setMapParameters,
  ...
  const changeGridSize = () => {
    let size = null;
    if (!mapRef.current || mapRef.current.zoom <= 10) size = 80;  // 低縮放時較大的聚合
    else if (mapRef.current.zoom <= 12) size = 60;
    else if (mapRef.current.zoom <= 15) size = 40;
    else if (mapRef.current.zoom <= 18) size = 20;  // 高縮放時較小的聚合
    else size = 1;  // 最高縮放時幾乎不聚合
    setGridSize(size);
  };
        ...
        (i) =>
          ...
          constructionsData[Math.floor(i / 10)][i % 10].coordinate.lng !==
            ...
            <Marker
              ...
              options={{
                markerId: i  // 添加標記 ID 以便在聚合中識別
              }}
              onClick={() => {
                console.log('🎯 Marker clicked! Index:', i);
                console.log('🎯 Marker data:', constructionsData[Math.floor(i / 10)][i % 10]);
                handleMarkerClick(i);
              }}
              clusterer={cluster}
              noClustererRedraw={true}
              icon={
                constructionsData[Math.floor(i / 10)][i % 10].status ===
                "OPEN"
                  ? {
